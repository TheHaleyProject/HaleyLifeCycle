    ...
    public partial class StateMachineMariaRepo {
        ...
        public async Task<IFeedback<long>> RegisterDefinition(string displayName, string description, int env) {
            ...
            try {
                var result = await _agw.Scalar(
                    new AdapterArgs(_key) { Query = QRY_DEFINITION.INSERT },
        ...
        public async Task<IFeedback<long>> RegisterDefinitionVersion(long parentId, int version, string jsonData) {
            ...
            try {
                ...
                var result = await _agw.Scalar(
                    new AdapterArgs(_key) { Query = QRY_DEF_VERSION.INSERT },
                    (PARENT, parentId),
                    (VERSION, version),
                    (DATA, jsonData)
                );
        ...
        public async Task<IFeedback<Dictionary<string, object>>> GetDefinitionById(long id) {
            ...
            try {
                var data = await _agw.Read(
                    new AdapterArgs(_key) { Query = QRY_DEFINITION.GET_BY_ID, Filter = ResultFilter.FirstDictionary },
                    (ID, id)
                );
        ...
        public async Task<IFeedback<List<Dictionary<string, object>>>> GetVersionsByDefinition(long definitionId) {
            ...
            try {
                var result = await _agw.Read(
                    new AdapterArgs(_key) { Query = QRY_DEF_VERSION.GET_BY_PARENT },
                    (PARENT, definitionId)
                );
            ...
            try {
                var data = await _agw.Read(
                    new AdapterArgs(_key) {
                        Query = "SELECT * FROM def_version WHERE parent = @PARENT ORDER BY version DESC LIMIT 1;",
        ...
        public async Task<IFeedback<bool>> DefinitionExists(string displayName, int env) {
            ...
            try {
                var result = await _agw.Scalar(
                    new AdapterArgs(_key) {
                        Query = "SELECT id FROM definition WHERE display_name = @DISPLAY_NAME AND env = @ENV LIMIT 1;"
                    },
                    (DISPLAY_NAME, displayName),
                    (ENV, env)
                );
        ...
        public async Task<IFeedback<bool>> UpdateDefinitionDescription(long definitionId, string newDescription) {
            ...
            try {
                await _agw.NonQuery(
                    new AdapterArgs(_key) {
                        Query = "UPDATE definition SET description = @DESCRIPTION WHERE id = @ID;"
                    },
        ...
        public async Task<IFeedback<bool>> DeleteDefinition(long definitionId) {
            ...
            try {
                await _agw.NonQuery(
                    new AdapterArgs(_key) { Query = QRY_DEFINITION.DELETE },
                    (ID, definitionId)
                );
